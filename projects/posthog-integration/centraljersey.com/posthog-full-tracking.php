<?php
/**
 * PostHog Full Tracking - centraljersey.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function centraljersey_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_DJse5iNBxxftS8JLYNYxcho9pKfIYgfJIqv6fzaAaAR', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for centraljersey.com');
            }
        });
    </script>
...
add_action('wp_head', 'centraljersey_com_posthog_tracking', 1);
 ...
 */
/*
function centraljersey_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'centraljersey_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function centraljersey_com_woocommerce_conversions() {
...
add_action('wp_footer', 'centraljersey_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function centraljersey_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'centraljersey_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function centraljersey_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'centraljersey_com_search_tracking');
