<?php
/**
 * PostHog Full Tracking - thesunpapers.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function thesunpapers_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_2c5WwvvcVY7GfaZtxBHDVxO0p7hEkEGlx1CIHOtcLM4', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for thesunpapers.com');
            }
        });
    </script>
...
add_action('wp_head', 'thesunpapers_com_posthog_tracking', 1);
 ...
 */
/*
function thesunpapers_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'thesunpapers_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function thesunpapers_com_woocommerce_conversions() {
...
add_action('wp_footer', 'thesunpapers_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function thesunpapers_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'thesunpapers_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function thesunpapers_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'thesunpapers_com_search_tracking');
