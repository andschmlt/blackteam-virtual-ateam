<?php
/**
 * PostHog Full Tracking - sport-oesterreich.at
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function sport_oesterreich_at_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_W3zqHnAcHFgCNiPboYEUi9jXz8sSyBSmY6VmUjV69Sy', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for sport-oesterreich.at');
            }
        });
    </script>
...
add_action('wp_head', 'sport_oesterreich_at_posthog_tracking', 1);
 ...
 */
/*
function sport_oesterreich_at_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'sport_oesterreich_at_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function sport_oesterreich_at_woocommerce_conversions() {
...
add_action('wp_footer', 'sport_oesterreich_at_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function sport_oesterreich_at_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'sport_oesterreich_at_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function sport_oesterreich_at_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'sport_oesterreich_at_search_tracking');
