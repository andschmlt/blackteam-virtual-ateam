<?php
/**
 * PostHog Full Tracking - ostexperte.de
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function ostexperte_de_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_x1c84l3TE4DFT9aC8FiVPPhZKY1hxLeayEcBR8MrTiH', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for ostexperte.de');
            }
        });
    </script>
...
add_action('wp_head', 'ostexperte_de_posthog_tracking', 1);
 ...
 */
/*
function ostexperte_de_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'ostexperte_de_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function ostexperte_de_woocommerce_conversions() {
...
add_action('wp_footer', 'ostexperte_de_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function ostexperte_de_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'ostexperte_de_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function ostexperte_de_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'ostexperte_de_search_tracking');
