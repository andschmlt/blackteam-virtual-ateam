<?php
/**
 * PostHog + NavBoost Integration for ostexperte.de
 * PostHog Project ID: 295247
 * Site Type: News/Media
 * Generated: 2026-01-21
 */
if (!defined('ABSPATH')) exit;

function ostexperte_de_enqueue_posthog_scripts() {
...
add_action('wp_enqueue_scripts', 'ostexperte_de_enqueue_posthog_scripts');

function ostexperte_de_add_body_class($classes) {
...
add_filter('body_class', 'ostexperte_de_add_body_class');

function ostexperte_de_track_article_meta() {
    if (!is_single()) return;
    global $post;
    ...
    <script>
        if (typeof posthog !== 'undefined') {
            posthog.register({
                ...
                'content_vertical': 'international_news'
            });
        }
    </script>
    <?php
}
add_action('wp_footer', 'ostexperte_de_track_article_meta');
