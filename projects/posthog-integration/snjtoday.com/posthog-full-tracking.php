<?php
/**
 * PostHog Full Tracking - snjtoday.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function snjtoday_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_A61d3raTTKbqZOLuRh5hSEtUXzaZ5EPtDCrfBCxQIBu', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for snjtoday.com');
            }
        });
    </script>
...
add_action('wp_head', 'snjtoday_com_posthog_tracking', 1);
 ...
 */
/*
function snjtoday_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'snjtoday_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function snjtoday_com_woocommerce_conversions() {
...
add_action('wp_footer', 'snjtoday_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function snjtoday_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'snjtoday_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function snjtoday_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'snjtoday_com_search_tracking');
