<?php
/**
 * PostHog Full Tracking - bestdaily.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function bestdaily_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_ybDGSHHEW73FGc79N5zxHercQ9jqmBNpI47uh290MYp', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for bestdaily.com');
            }
        });
    </script>
...
add_action('wp_head', 'bestdaily_com_posthog_tracking', 1);
 ...
 */
/*
function bestdaily_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'bestdaily_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function bestdaily_com_woocommerce_conversions() {
...
add_action('wp_footer', 'bestdaily_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function bestdaily_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'bestdaily_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function bestdaily_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'bestdaily_com_search_tracking');
