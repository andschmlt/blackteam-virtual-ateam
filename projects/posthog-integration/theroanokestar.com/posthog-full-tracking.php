<?php
/**
 * PostHog Full Tracking - theroanokestar.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function theroanokestar_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_w3UubjrNc7OTuqLfaefR4xlnCiJTfue23FLImnFfvBd', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for theroanokestar.com');
            }
        });
    </script>
...
add_action('wp_head', 'theroanokestar_com_posthog_tracking', 1);
 ...
 */
/*
function theroanokestar_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'theroanokestar_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function theroanokestar_com_woocommerce_conversions() {
...
add_action('wp_footer', 'theroanokestar_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function theroanokestar_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'theroanokestar_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function theroanokestar_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'theroanokestar_com_search_tracking');
