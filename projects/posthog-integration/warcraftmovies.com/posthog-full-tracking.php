<?php
/**
 * PostHog Full Tracking - warcraftmovies.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function warcraftmovies_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_6SJcgZAxlG6ahPTBIfu894Mm0h6omEmr4AjESqkmujN', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for warcraftmovies.com');
            }
        });
    </script>
...
add_action('wp_head', 'warcraftmovies_com_posthog_tracking', 1);
 ...
 */
/*
function warcraftmovies_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'warcraftmovies_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function warcraftmovies_com_woocommerce_conversions() {
...
add_action('wp_footer', 'warcraftmovies_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function warcraftmovies_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'warcraftmovies_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function warcraftmovies_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'warcraftmovies_com_search_tracking');
