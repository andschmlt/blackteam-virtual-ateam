<?php
/**
 * PostHog + NavBoost Integration for management.org
 * PostHog Project ID: 295243
 * Site Type: Business/Professional
 * Generated: 2026-01-21
 */
if (!defined('ABSPATH')) exit;

function management_org_enqueue_posthog_scripts() {
...
add_action('wp_enqueue_scripts', 'management_org_enqueue_posthog_scripts');

function management_org_add_body_class($classes) {
...
add_filter('body_class', 'management_org_add_body_class');

function management_org_track_article_meta() {
    if (!is_single()) return;
    global $post;
    ...
    <script>
        if (typeof posthog !== 'undefined') {
            posthog.register({
                ...
                'content_vertical': 'business'
            });
        }
    </script>
    <?php
}
add_action('wp_footer', 'management_org_track_article_meta');
