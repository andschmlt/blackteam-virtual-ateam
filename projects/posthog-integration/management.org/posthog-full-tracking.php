<?php
/**
 * PostHog Full Tracking - management.org
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function management_org_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_6KHNR8kD2b5GNeyaoIBVgwVrqvsT3Ofs1T5s38bfbsW', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for management.org');
            }
        });
    </script>
...
add_action('wp_head', 'management_org_posthog_tracking', 1);
 ...
 */
/*
function management_org_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'management_org_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function management_org_woocommerce_conversions() {
...
add_action('wp_footer', 'management_org_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function management_org_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'management_org_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function management_org_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'management_org_search_tracking');
