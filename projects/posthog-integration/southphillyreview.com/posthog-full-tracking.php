<?php
/**
 * PostHog Full Tracking - southphillyreview.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function southphillyreview_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_R0tICFsX9Uxm2g64wUoSsV1Nye6Qok3OXJ6UZUneNT8', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for southphillyreview.com');
            }
        });
    </script>
...
add_action('wp_head', 'southphillyreview_com_posthog_tracking', 1);
 ...
 */
/*
function southphillyreview_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'southphillyreview_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function southphillyreview_com_woocommerce_conversions() {
...
add_action('wp_footer', 'southphillyreview_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function southphillyreview_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'southphillyreview_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function southphillyreview_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'southphillyreview_com_search_tracking');
