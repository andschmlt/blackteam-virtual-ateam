<?php
/**
 * PostHog Full Tracking - betanews.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function betanews_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_WHI3lhB9h53h7cMAZt8nkQ2WT5ncKoFsvM3LkHawGKL', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for betanews.com');
            }
        });
    </script>
...
add_action('wp_head', 'betanews_com_posthog_tracking', 1);
 ...
 */
/*
function betanews_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'betanews_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function betanews_com_woocommerce_conversions() {
...
add_action('wp_footer', 'betanews_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function betanews_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'betanews_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function betanews_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'betanews_com_search_tracking');
