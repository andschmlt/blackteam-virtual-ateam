<?php
/**
 * PostHog + NavBoost Integration for pokertube.com
...
function pokertube_com_enqueue_posthog_scripts() {
...
add_action('wp_enqueue_scripts', 'pokertube_com_enqueue_posthog_scripts');

function pokertube_com_add_body_class($classes) {
...
add_filter('body_class', 'pokertube_com_add_body_class');

function pokertube_com_track_article_meta() {
    if (!is_single()) return;
    global $post;
    ...
    <script>
        if (typeof posthog !== 'undefined') {
            posthog.register({
                ...
                'content_vertical': 'poker'
            });
        }
    </script>
    <?php
}
add_action('wp_footer', 'pokertube_com_track_article_meta');
