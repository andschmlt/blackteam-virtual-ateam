<?php
/**
 * PostHog Full Tracking - pokertube.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function pokertube_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_lVP4GZkbFmBzbkhY8x4xTWqXtT6MwTLKboGUcUW0thW', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for pokertube.com');
            }
        });
    </script>
...
add_action('wp_head', 'pokertube_com_posthog_tracking', 1);
 ...
 */
/*
function pokertube_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'pokertube_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function pokertube_com_woocommerce_conversions() {
...
add_action('wp_footer', 'pokertube_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function pokertube_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'pokertube_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function pokertube_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'pokertube_com_search_tracking');
