<?php
/**
 * PostHog Full Tracking - pokerology.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function pokerology_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_yL0Kmph4HtqQpW47JtiDnbowvmbMqVVZ0eVU4ayuFmx', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for pokerology.com');
            }
        });
    </script>
...
add_action('wp_head', 'pokerology_com_posthog_tracking', 1);
 ...
 */
/*
function pokerology_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'pokerology_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function pokerology_com_woocommerce_conversions() {
...
add_action('wp_footer', 'pokerology_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function pokerology_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'pokerology_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function pokerology_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'pokerology_com_search_tracking');
