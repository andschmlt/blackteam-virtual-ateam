/**
 * NavBoost Tracker - pokerology.com
 * Site Type: Affiliate/Casino
 * Vertical: affiliate
 * PostHog Project ID: 294549
...
(function() {
    ...
    // ==========================================
    const CONFIG = {
        SITE_DOMAIN: 'pokerology.com',
        SITE_TYPE: 'Affiliate/Casino',
        VERTICAL: 'affiliate',
    ...
    // ==========================================
    // TOPLIST ROW VISIBILITY (Affiliate Sites)
    // Per NavBoost KPI Framework - Secondary KPI #7
    // ==========================================
    function initToplistTracking() {
        const toplistSelectors = [
            '.toplist-row',
            '.comparison-row',
            '.poker-room-row',
            '.casino-row',
        ...
        const toplistObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    const rowId = row.dataset.rowId || row.id || `row-${rowIndex}`;
                    
                    if (!state.toplistRowsVisible.has(rowId)) {
                        state.toplistRowsVisible.add(rowId);
                        state.toplistRowsCount++;
                        
                        trackEvent('navboost:toplist_row_visible', {
                            row_position: rowIndex,
                            row_id: rowId,
                            row_brand: row.dataset.brand || row.querySelector('.brand-name')?.textContent?.trim() || null,
                            total_rows_visible: state.toplistRowsCount,
                            time_to_visibility: Math.round((Date.now() - state.landingTimestamp) / 1000)
                        });
                    }
                }
        ...
        toplistSelectors.forEach(selector => {
            try {
                document.querySelectorAll(selector).forEach(row => toplistObserver.observe(row));
            } catch (e) { log('Invalid toplist selector:', selector); }
        });
        
        log('Toplist tracking initialized');
    }
    ...
    function init() {
        ...
        log('Initializing NavBoost Tracker for pokerology.com');
