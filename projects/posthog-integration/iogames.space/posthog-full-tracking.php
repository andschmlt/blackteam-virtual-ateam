<?php
/**
 * PostHog Full Tracking - iogames.space
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function iogames_space_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_p3kYqZKK5kPxxhyfGkQv5wEgm02zg4iuIzLr0AIUOb9', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for iogames.space');
            }
        });
    </script>
...
add_action('wp_head', 'iogames_space_posthog_tracking', 1);
 ...
 */
/*
function iogames_space_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'iogames_space_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function iogames_space_woocommerce_conversions() {
...
add_action('wp_footer', 'iogames_space_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function iogames_space_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'iogames_space_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function iogames_space_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'iogames_space_search_tracking');
