<?php
/**
 * PostHog Full Tracking - europeangaming.eu
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function europeangaming_eu_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_yL0Kmph4HtqQpW47JtiDnbowvmbMqVVZ0eVU4ayuFmx', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for europeangaming.eu');
            }
        });
    </script>
...
add_action('wp_head', 'europeangaming_eu_posthog_tracking', 1);
 ...
 */
/*
function europeangaming_eu_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'europeangaming_eu_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function europeangaming_eu_woocommerce_conversions() {
...
add_action('wp_footer', 'europeangaming_eu_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function europeangaming_eu_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'europeangaming_eu_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function europeangaming_eu_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'europeangaming_eu_search_tracking');
