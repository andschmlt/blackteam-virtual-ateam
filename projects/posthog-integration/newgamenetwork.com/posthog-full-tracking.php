<?php
/**
 * PostHog Full Tracking - newgamenetwork.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function newgamenetwork_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_7i9SHrMx0XXb1C8OzK8DoL6zIxS5tjLHQSE6xHwc7ZY', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for newgamenetwork.com');
            }
        });
    </script>
...
add_action('wp_head', 'newgamenetwork_com_posthog_tracking', 1);
 ...
 */
/*
function newgamenetwork_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'newgamenetwork_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function newgamenetwork_com_woocommerce_conversions() {
...
add_action('wp_footer', 'newgamenetwork_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function newgamenetwork_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'newgamenetwork_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function newgamenetwork_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'newgamenetwork_com_search_tracking');
