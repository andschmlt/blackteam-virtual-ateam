<?php
/**
 * PostHog Full Tracking - esports.gg
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function esports_gg_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_UHCLBas5g7FvJMytBR30C3IMyOLs4KJ90t1QY8N9tKx', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for esports.gg');
            }
        });
    </script>
...
add_action('wp_head', 'esports_gg_posthog_tracking', 1);
 ...
 */
/*
function esports_gg_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'esports_gg_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function esports_gg_woocommerce_conversions() {
...
add_action('wp_footer', 'esports_gg_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function esports_gg_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'esports_gg_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function esports_gg_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'esports_gg_search_tracking');
