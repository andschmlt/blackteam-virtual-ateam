<?php
/**
 * PostHog + NavBoost Integration for esports.gg
 *
 ...
 */
function esports_gg_enqueue_posthog_scripts() {
    ...
    add_action('wp_head', function() {
        ...
        posthog.init('phc_UHCLBas5g7FvJMytBR30C3IMyOLs4KJ90t1QY8N9tKx', {
...
add_action('wp_enqueue_scripts', 'esports_gg_enqueue_posthog_scripts');

/**
 * Add page template data attribute for NavBoost
 */
function esports_gg_add_body_class($classes) {
...
add_filter('body_class', 'esports_gg_add_body_class');

/**
 * Add data attributes to CTAs for better tracking
 */
function esports_gg_add_cta_attributes($content) {
...
add_filter('the_content', 'esports_gg_add_cta_attributes');

/**
 * Track article metadata for better analytics
 */
function esports_gg_track_article_meta() {
    if (!is_single()) {
        return;
    }
...
add_action('wp_footer', 'esports_gg_track_article_meta');
