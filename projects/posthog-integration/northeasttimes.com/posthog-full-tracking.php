<?php
/**
 * PostHog Full Tracking - northeasttimes.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function northeasttimes_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_yL0Kmph4HtqQpW47JtiDnbowvmbMqVVZ0eVU4ayuFmx', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for northeasttimes.com');
            }
        });
    </script>
...
add_action('wp_head', 'northeasttimes_posthog_tracking', 1);
 ...
 */
/*
function northeasttimes_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'northeasttimes_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function northeasttimes_woocommerce_conversions() {
...
add_action('wp_footer', 'northeasttimes_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function northeasttimes_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'northeasttimes_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function northeasttimes_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'northeasttimes_search_tracking');
