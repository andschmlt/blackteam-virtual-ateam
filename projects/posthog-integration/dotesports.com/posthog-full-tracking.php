<?php
/**
 * PostHog Full Tracking - dotesports.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function dotesports_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_8jgdrakp5mAyOcX6sgYFyoNXLOyzqh4b4MDj6rUQubX', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for dotesports.com');
            }
        });
    </script>
...
add_action('wp_head', 'dotesports_com_posthog_tracking', 1);
 ...
 */
/*
function dotesports_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'dotesports_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function dotesports_com_woocommerce_conversions() {
...
add_action('wp_footer', 'dotesports_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function dotesports_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'dotesports_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function dotesports_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'dotesports_com_search_tracking');
