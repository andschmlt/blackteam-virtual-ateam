<?php
/**
 * PostHog + NavBoost Integration for dotesports.com
 *
 * Add this code to your theme's functions.php file
 * OR create a custom plugin with this code
 *
 * PostHog Project ID: 291573
 ...
 */
function dotesports_com_enqueue_posthog_scripts() {
    ...
    add_action('wp_head', function() {
        ...
        posthog.init('phc_8jgdrakp5mAyOcX6sgYFyoNXLOyzqh4b4MDj6rUQubX', {
    ...
    wp_enqueue_script(
        'navboost-tracker', get_template_directory_uri() . '/js/navboost-tracker.js', array(), '1.3.1',
        true // Load in footer
    );
}
add_action('wp_enqueue_scripts', 'dotesports_com_enqueue_posthog_scripts');

/**
 * Add page template data attribute for NavBoost
 */
function dotesports_com_add_body_class($classes) {
    ...
    if (is_front_page()) {
        ...
        // Check for esports-specific pages
        if (is_page('esports') || is_page('tournaments')) {
            $classes[] = 'template-esports';
        }
        if (is_page('guides') || is_page('tier-lists')) {
            $classes[] = 'template-guides';
        }
    }

    return $classes;
}
add_filter('body_class', 'dotesports_com_add_body_class');

/**
 * Add data attributes to CTAs for better tracking
 */
function dotesports_com_add_cta_attributes($content) {
    ...
    $patterns = array(
        ...
        // Guide links
        '/<a([^>]*class="[^"]*guide[^"]*"[^>]*)>/i' => '<a$1 data-cta-type="guide">',
    );
...
add_filter('the_content', 'dotesports_com_add_cta_attributes');

/**
 * Track article metadata for better analytics
 */
function dotesports_com_track_article_meta() {
    if (!is_single()) {
        return;
    }
    ...
    <script>
        if (typeof posthog !== 'undefined') {
            posthog.register({
                ...
                'content_vertical': 'esports'
            });
        }
    </script>
    <?php
}
add_action('wp_footer', 'dotesports_com_track_article_meta');
