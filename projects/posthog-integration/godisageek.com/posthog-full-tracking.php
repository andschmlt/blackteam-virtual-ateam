<?php
/**
 * PostHog Full Tracking - godisageek.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function godisageek_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_LAFbPCUQJXmQ0KpWjasr1VdddjGFdlzQFyHVjVW6iMB', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for godisageek.com');
            }
        });
    </script>
...
add_action('wp_head', 'godisageek_com_posthog_tracking', 1);
 ...
 */
/*
function godisageek_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'godisageek_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function godisageek_com_woocommerce_conversions() {
...
add_action('wp_footer', 'godisageek_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function godisageek_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'godisageek_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function godisageek_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'godisageek_com_search_tracking');
