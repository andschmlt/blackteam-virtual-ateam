<?php
/**
 * PostHog Full Tracking - mrracy.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function mrracy_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_WOZPaFNksoTFYQu6bmGrSqxAsq1C9ph3DHVboKEcBLT', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for mrracy.com');
            }
        });
    </script>
...
add_action('wp_head', 'mrracy_com_posthog_tracking', 1);
 ...
 */
/*
function mrracy_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'mrracy_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function mrracy_com_woocommerce_conversions() {
...
add_action('wp_footer', 'mrracy_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function mrracy_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'mrracy_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function mrracy_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'mrracy_com_search_tracking');
