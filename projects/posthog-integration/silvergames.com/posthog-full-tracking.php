<?php
/**
 * PostHog Full Tracking - silvergames.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function silvergames_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_yHsK3TNqaeIk8PbIIu0VqOuXY19H2kChIbbpS6ApcOD', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for silvergames.com');
            }
        });
    </script>
...
add_action('wp_head', 'silvergames_com_posthog_tracking', 1);
 ...
 */
/*
function silvergames_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'silvergames_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function silvergames_com_woocommerce_conversions() {
...
add_action('wp_footer', 'silvergames_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function silvergames_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'silvergames_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function silvergames_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'silvergames_com_search_tracking');
