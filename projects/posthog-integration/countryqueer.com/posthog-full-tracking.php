<?php
/**
 * PostHog Full Tracking - countryqueer.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function countryqueer_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_fscF05W6W8u3PhRgY0O3t8gaSsCYczTrPuUT7TBqIt5', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for countryqueer.com');
            }
        });
    </script>
...
add_action('wp_head', 'countryqueer_com_posthog_tracking', 1);
 ...
 */
/*
function countryqueer_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'countryqueer_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function countryqueer_com_woocommerce_conversions() {
...
add_action('wp_footer', 'countryqueer_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function countryqueer_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'countryqueer_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function countryqueer_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'countryqueer_com_search_tracking');
