<?php
/**
 * PostHog Full Tracking - hudsonreporter.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function hudsonreporter_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
...
add_action('wp_head', 'hudsonreporter_posthog_tracking', 1);
 ...
 */
/*
function hudsonreporter_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'hudsonreporter_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function hudsonreporter_woocommerce_conversions() {
...
add_action('wp_footer', 'hudsonreporter_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function hudsonreporter_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'hudsonreporter_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function hudsonreporter_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'hudsonreporter_search_tracking');
