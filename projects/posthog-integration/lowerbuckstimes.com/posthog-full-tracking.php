<?php
/**
 * PostHog Full Tracking - lowerbuckstimes.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function lowerbuckstimes_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_GfmQiWa33XhxjjMOT5DUtdftF1kF2zd12GGHU607ze8', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for lowerbuckstimes.com');
            }
        });
    </script>
...
add_action('wp_head', 'lowerbuckstimes_com_posthog_tracking', 1);
 ...
 */
/*
function lowerbuckstimes_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'lowerbuckstimes_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function lowerbuckstimes_com_woocommerce_conversions() {
...
add_action('wp_footer', 'lowerbuckstimes_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function lowerbuckstimes_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'lowerbuckstimes_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function lowerbuckstimes_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'lowerbuckstimes_com_search_tracking');
