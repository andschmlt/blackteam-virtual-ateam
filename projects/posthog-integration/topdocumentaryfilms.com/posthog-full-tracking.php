<?php
/**
 * PostHog Full Tracking - topdocumentaryfilms.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function topdocumentaryfilms_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_hMhLAhMtJn35IKWnYw0EAN54EbaWGIC35GSNTQgKTeY', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for topdocumentaryfilms.com');
            }
        });
    </script>
...
add_action('wp_head', 'topdocumentaryfilms_com_posthog_tracking', 1);
 ...
 */
/*
function topdocumentaryfilms_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'topdocumentaryfilms_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function topdocumentaryfilms_com_woocommerce_conversions() {
...
add_action('wp_footer', 'topdocumentaryfilms_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function topdocumentaryfilms_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'topdocumentaryfilms_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function topdocumentaryfilms_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'topdocumentaryfilms_com_search_tracking');
