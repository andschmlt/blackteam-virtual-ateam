<?php
/**
 * PostHog + NavBoost Integration for topdocumentaryfilms.com
 * PostHog Project ID: 295257
 * Site Type: Entertainment
 * Generated: 2026-01-21
 */
if (!defined('ABSPATH')) exit;

function topdocumentaryfilms_com_enqueue_posthog_scripts() {
...
add_action('wp_enqueue_scripts', 'topdocumentaryfilms_com_enqueue_posthog_scripts');

function topdocumentaryfilms_com_add_body_class($classes) {
...
add_filter('body_class', 'topdocumentaryfilms_com_add_body_class');

function topdocumentaryfilms_com_track_article_meta() {
    if (!is_single()) return;
    global $post;
    ...
    <script>
        if (typeof posthog !== 'undefined') {
            posthog.register({
                ...
                'content_vertical': 'documentary'
            });
        }
    </script>
    <?php
}
add_action('wp_footer', 'topdocumentaryfilms_com_track_article_meta');
