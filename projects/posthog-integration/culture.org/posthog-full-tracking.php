<?php
/**
 * PostHog Full Tracking - culture.org
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function culture_org_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_WCW3bwmRex1An9pjMFMBFAvVLTrq9I4UVE3PA6yGDqK', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for culture.org');
            }
        });
    </script>
...
add_action('wp_head', 'culture_org_posthog_tracking', 1);
 ...
 */
/*
function culture_org_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'culture_org_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function culture_org_woocommerce_conversions() {
...
add_action('wp_footer', 'culture_org_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function culture_org_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'culture_org_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function culture_org_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'culture_org_search_tracking');
