<?php
/**
 * PostHog + NavBoost Integration for culture.org
 * PostHog Project ID: 295239
...
function culture_org_add_body_class($classes) {
...
add_filter('body_class', 'culture_org_add_body_class');

function culture_org_track_article_meta() {
    if (!is_single()) return;
    global $post;
    ...
    <script>
        if (typeof posthog !== 'undefined') {
            posthog.register({
                ...
                'content_vertical': 'culture'
            });
        }
    </script>
    <?php
}
add_action('wp_footer', 'culture_org_track_article_meta');
