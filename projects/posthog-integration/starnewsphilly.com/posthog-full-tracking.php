<?php
/**
 * PostHog Full Tracking - starnewsphilly.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function starnewsphilly_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_PClZUzvjDoWPBNJvaLkVPB779XEE82aQfSkIuMQnUsW', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for starnewsphilly.com');
            }
        });
    </script>
...
add_action('wp_head', 'starnewsphilly_com_posthog_tracking', 1);
 ...
 */
/*
function starnewsphilly_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'starnewsphilly_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function starnewsphilly_com_woocommerce_conversions() {
...
add_action('wp_footer', 'starnewsphilly_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function starnewsphilly_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'starnewsphilly_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function starnewsphilly_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'starnewsphilly_com_search_tracking');
