<?php
/**
 * PostHog Full Tracking - metrotimes.com
 *
 * Includes:
 * - PostHog SDK
 * - NavBoost Tracker
 ...
 */
function metrotimes_com_posthog_tracking() {
    ?>
    <!-- PostHog SDK -->
    <script>
        ...
        posthog.init('phc_bac3vRylsrdbqYnOmVz9yAxCIlUSPWIRRAcs6qJ5OL5', {
            ...
            loaded: function(posthog) {
                console.log('[PostHog] Loaded for metrotimes.com');
            }
        });
    </script>
...
add_action('wp_head', 'metrotimes_com_posthog_tracking', 1);
 ...
 */
/*
function metrotimes_com_posthog_tracking_external() {
    // PostHog SDK
    wp_enqueue_script(
        'posthog-sdk',
...
add_action('wp_enqueue_scripts', 'metrotimes_com_posthog_tracking_external');
*/

/**
 * Add custom conversion tracking for WooCommerce (if applicable)
 */
function metrotimes_com_woocommerce_conversions() {
...
add_action('wp_footer', 'metrotimes_com_woocommerce_conversions');

/**
 * Track comment submissions as conversions
 */
function metrotimes_com_comment_conversion($comment_id, $comment_approved) {
    if ($comment_approved) {
        ?>
        <script>
        if (typeof posthog !== 'undefined') {
...
add_action('comment_post', 'metrotimes_com_comment_conversion', 10, 2);

/**
 * Track search queries
 */
function metrotimes_com_search_tracking() {
    if (is_search()) {
...
add_action('wp_footer', 'metrotimes_com_search_tracking');
